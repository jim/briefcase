...
With shhh, dotfiles are stored in a centralized location (by default ~/.dotfiles), and symlinks are created for these files in the user's home directory. Here is a basic setup for a .gitconfig file:
    ...
    | | +-gitconfig.dynamic   Classified dotfile
...
==== Classified dotfiles
A config file that contains some secret information. The file is stored, sans secret info, with a '.classified' extension in the repo. The dotfile that is symlinked to the user's home directory is then generated from this file.
...
This file is then saved with a '.classified' extension, and the original file is added to ~/.dotfiles/.gitignore so it will not be added to the repository.
...
=== generate
Creates local version of a classified dotfile, using information found in the secrets file to fill in any that was removed.
...
* 0.2.0 Added redact command, use .classified for dynamic dotfiles
