...
describe Shhh::Commands::Import do
  ...
  describe "collision handling" do
    ...
    before do
      @original = File.join(home_path, '.test')
      @moved = File.join(dotfiles_path, 'test')
      @relocated = File.join(dotfiles_path, 'test.old.1')
    ...
    it "renames an existing dotfile when importing a duplicate and instructed to replace it" do
      ...
      run_command("import #{@original}") do |c|
    ...
    it "renames an existing duplicate dotfile  when importing a duplicate and instructed to replace it" do
      ...
      create_empty_file(@relocated)
    
      run_command("import #{@original}") do |c|
        c.response(/Do you want to replace it\?/, 'replace')
      end
      
      duplicate = File.join(dotfiles_path, 'test.old.2')
      
      file_must_have_moved(@moved, duplicate)
      
      file_must_exist(duplicate)
      file_must_not_have_moved(@relocated)
    end
